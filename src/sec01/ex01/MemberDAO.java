...
public class MemberDAO {
	...
	public MemberDAO() {
		try{
			...
			//lookup("java:/comp/env")는 그중 환경설정에 관련된 컨텍스트 객체에 접근하기 위한 기본주소입니다.
			Context envContext = (Context)ctx.lookup("java:/comp/env");
			
			//그런후 톰캣 context.xml에 설정한 <Resource name="jdbc/oracle"..> 태그의
			//name값 "jdbc/oracle"를 이용해 톰캣이 미리 DB에 연결 해놓은 DataSource객체(커넥션 풀역할을 하는 객체)를
			//받아옵니다
	...
	//DB에 회원정보를 추가할 메소드
	//member.jsp페이지에서 호출하는 메소드로 매개변수로 전달 되어 오는 MemberBean객체의 각변수에는?
	//입력한 회원정보들이 저장되어 있다.
	public void addMember(MemberBean memberBean){
		
		try{
				//DataSource(커넥션풀)내부에 있는 DB와 미리연결을 맺은 Connection객체 꺼내어 빌려오기
			...
			//insert문장 만들기 (PreparedStatememt방식)
			String query = "insert into t_member(id,pwd,name,email)values(?,?,?,?)";
			
			//?를 제외한 insert문장을 임시로 OraclePreparedStatementWrapper실행객체에 담아
			//OraclePreparedStatementWrapper <-- insert문장을 DB에 전송하여 실행할 역할을 하는 객체 얻기
	...
	//DB의 모든 회원정보를 조회(검색) 하는 메소드
		...
		try{
			...
			//테이블 형식의 구조를 가지고 있는 OracleResultSetImpI객체에 조회한 회원레코드가 존재하는 동안 반복
			while(rs.next()){
			...
			//그후 MemberBean객체를 ArraryList배열에 추가	
			list.add(vo);
